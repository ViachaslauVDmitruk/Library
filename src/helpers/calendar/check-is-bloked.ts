...
export const checkIsBlockedDate = (date: ReturnType<typeof createDate>) => {
  ...
  const isTodayFriday = todayDate.dayNumberInWeek === 6 && date.timestamp > todayDate.timestamp + dayMs * 3;

  const isTodaySaturday = todayDate.dayNumberInWeek === 7 && date.timestamp > todayDate.timestamp + dayMs * 2;

  const isTodaySunday = todayDate.dayNumberInWeek === 1 && date.timestamp > todayDate.timestamp + dayMs;
  ...
  return isLessThanToday || isTomorrow || isTodayFriday || isTodaySunday || isTodaySaturday || isCurrentDateIsWeekend;
};
